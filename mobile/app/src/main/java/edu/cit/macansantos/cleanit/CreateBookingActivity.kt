...
import edu.cit.macansantos.cleanit.adapter.AddOnsAdapter
import edu.cit.macansantos.cleanit.adapter.TechniciansAdapter
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@CreateBookingActivity, 
                        "Failed to load service", Toast.LENGTH_SHORT).show()
                    finish()
                }
            } catch (e: Exception) {
                Toast.makeText(this@CreateBookingActivity, 
        ...
        lifecycleScope.launch {
            try {
                val bookingData = mapOf(
                    "clientId" to userId,
                    "technicianId" to selectedTechnicianId!!,
                    "serviceId" to service!!.id,
                    "serviceType" to service!!.name,
                    "deviceType" to deviceType,
                    "addOns" to selectedAddOns.map { it.id },
                    "timeSlot" to timeSlot,
                    "bookingDate" to bookingDate,
                    "address" to address,
                    "landmark" to etLandmark.text.toString().trim(),
                    "specialInstructions" to etSpecialInstructions.text.toString().trim(),
                    "totalAmount" to (service!!.basePrice + selectedAddOns.sumOf { it.price })
                )
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@CreateBookingActivity, 
                        "Failed to create booking", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@CreateBookingActivity, 
                    "Error: ${e.message}", Toast.LENGTH_SHORT).show()
