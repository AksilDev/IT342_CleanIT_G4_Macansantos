...
import com.G4.backend.features.booking.BookingService;
import com.G4.backend.shared.storage.SupabaseStorageService;
...
public class TechnicianBookingController {
    ...
    public ResponseEntity<?> getTechnicianBookings(@PathVariable UUID technicianId) {
        try {
            ...
            for (Booking booking : bookings) {
                ...
                if (booking.getStatus() != com.G4.backend.features.booking.BookingStatus.PENDING) {
    ...
    public ResponseEntity<?> updateBookingStatus(
        ...
        try {
            ...
            com.G4.backend.features.booking.BookingStatus newStatus = 
                com.G4.backend.features.booking.BookingStatus.fromValue(statusValue);
            
            com.G4.backend.features.booking.Booking updatedBooking = 
    ...
    public ResponseEntity<?> getTechnicianStatistics(@PathVariable UUID technicianId) {
        try {
            List<com.G4.backend.features.booking.Booking> allBookings = bookingService.getTechnicianBookings(technicianId);
            
            Map<String, Object> stats = new HashMap<>();
            
            // Count by status
            long completed = allBookings.stream()
                .mapToLong(b -> b.getStatus() == com.G4.backend.features.booking.BookingStatus.COMPLETED ? 1 : 0)
                .sum();
            long confirmed = allBookings.stream()
                .mapToLong(b -> b.getStatus() == com.G4.backend.features.booking.BookingStatus.CONFIRMED ? 1 : 0)
                .sum();
            long inProgress = allBookings.stream()
                .mapToLong(b -> b.getStatus() == com.G4.backend.features.booking.BookingStatus.IN_PROGRESS ? 1 : 0)
                .sum();
            ...
            double totalEarnings = allBookings.stream()
                .filter(b -> b.getStatus() == com.G4.backend.features.booking.BookingStatus.COMPLETED)
