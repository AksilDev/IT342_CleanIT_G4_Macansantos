...
public class AdminController {
    ...
    public ResponseEntity<?> getBookingsByStatus(@PathVariable String status) {
        try {
            com.G4.backend.features.booking.BookingStatus bookingStatus = 
                com.G4.backend.features.booking.BookingStatus.fromValue(status);
            
            List<com.G4.backend.features.booking.Booking> bookings = 
                bookingService.getBookingsByStatus(bookingStatus);
            
            List<Map<String, Object>> response = new ArrayList<>();
            for (com.G4.backend.features.booking.Booking booking : bookings) {
    ...
    public ResponseEntity<?> getBookingsByStatuses(@RequestParam String statuses) {
        try {
            ...
            List<com.G4.backend.features.booking.BookingStatus> statusList = new ArrayList<>();
            
            // Validate and convert each status string to BookingStatus enum
            for (String statusStr : statusArray) {
                try {
                    com.G4.backend.features.booking.BookingStatus bookingStatus = 
                        com.G4.backend.features.booking.BookingStatus.fromValue(statusStr.trim());
    ...
    public ResponseEntity<?> voidBooking(@PathVariable UUID bookingId) {
        ...
        try {
            ...
            com.G4.backend.features.users.User admin = (com.G4.backend.features.users.User) authentication.getPrincipal();
            ...
            com.G4.backend.features.booking.Booking cancelledBooking = bookingService.cancelBooking(
                bookingId, 
                admin.getId(), 
                reason
            );
        ...
        } catch (com.G4.backend.features.booking.BookingException e) {
