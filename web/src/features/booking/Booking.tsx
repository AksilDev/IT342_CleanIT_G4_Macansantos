...
export default function Booking() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!selectedDevice || !selectedTechnician || !selectedTimeSlot || !bookingDate || !address) {
		...
		<div className="min-h-screen bg-white">
			<div className="mx-auto max-w-2xl px-4 py-6">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					{/* Booking Date */}
					<div>
						<label className="text-xs font-semibold text-slate-700">Booking Date *</label>
						<input
							type="date"
							value={bookingDate}
							min={new Date().toISOString().split('T')[0]}
							onChange={(e) => setBookingDate(e.target.value)}
							className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-xs text-slate-700 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
							required
						/>
					</div>
